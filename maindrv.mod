...
$   IF SET TIMESTEP $SET R_T 'R,SUBT(T)' SET TX 'SUBT(T)' SET R_V_T 'R,V,SUBT(T)'
$   IF SET FIXBOH
$   IF DEFINED REG_FIXT $SET R_T 'RT_PP(%R_T%)' SET R_V_T 'RVT(%R_V_T%)' SET RTPX X
$   IFI %EXTSOL%==YES $SET SOLVE_NOW 'NO' SET REDUCE '0' SET VAR_UC 'YES' SET OBJANN 'YES' SET VARCOST LIN
*-----------------------------------------------------------------------------
* do an check on user data
*-----------------------------------------------------------------------------
...
*-----------------------------------------------------------------------------
$   IFI %STARTRUN%==RESTART $GOTO FROM_PREV
...
$   IF SET TIMESTEP          $SETLOCAL EXT stp
